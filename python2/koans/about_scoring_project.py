...
def is_five(item):
    return True if item == 5 else False#!/usr/bin/env python
# -*- coding: utf-8 -*-
...
def apply_triad_rules(srtd):
    if srtd.count(1) > 2:
        return remove_three_occurances_of_item_in_list(1, srtd), 1000
    srtset = set(srtd)
    if len(srtset) == (len(srtd) - 2):
        for item in srtset:
            if srtd.count(item) > 2:
                return remove_three_occurances_of_item_in_list(item, srtd), 100 * item
...
def score(dice):
    ...
    fltrd, score = apply_triad_rules(srtd)
